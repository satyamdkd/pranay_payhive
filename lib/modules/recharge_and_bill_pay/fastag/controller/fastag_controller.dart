...
class FastagController extends GetxController {
  final List<String> amounts = ["₹500", "₹750", "₹1000", "₹1500", "₹2000"];
  ...
  getAllElectricityBillers() async {
    ...
    try {
      final response = await repo!.getAllCreditCardBillers(catName: 'Fastag');
  ...
  Future<void> getFastagBillFetchResponse() async {
    await BBPSAuthToken.getBBPSAuthToken();
    repo = FastagRepo();
  ...
  double maxPermissibleAmount = 1000;
  ...
  void parseBillResponse(Map<String, dynamic> response) {
    try {
      ...
      if (data['additionalInfo'] != null) {
        for (var item in data['additionalInfo']) {
          if (item['name'].toString().toLowerCase() ==
            ...
            debugPrint(".\n\n\n\n${item['value']}\n\n\n\n.");
            maxPermissibleAmount = double.parse(item['value'].toString());
          }
          if (item['name'].toString().toLowerCase() == "wallet balance") {
            debugPrint(".\n\n\n\n${item['value']}\n\n\n\n.");
            walletBalance = double.parse(item['value'].toString());
          }
        }
      } else {
      ...
      for (var bill in bills) {
        ...
        /// Customer name if present "DL9CBK6235"
        final vehicleNumber = (bill['billNumber'] != null &&
                bill['billNumber'].toString().trim().isNotEmpty)
            ? bill['billNumber']
            : 'N/A';
        ...
        vehicleNumberFromApi = vehicleNumber;
