...
class GasController extends GetxController {
  ...
  TextEditingController svNumber = TextEditingController();
  ...
  getAllElectricityBillers() async {
    ...
    try {
      final response = await repo!.getAllCreditCardBillers(catName: 'LPG Gas');
    ...
    try {
      ...
      if (data['additionalInfo'] != null) {
        for (var item in data['additionalInfo']) {
          if (item['name'].toString().toLowerCase() == "consumer number") {
            debugPrint('Customer Name: ${item['name']}');
            consumerNumber = item['value'].toString();
          }
        }
      } else {
